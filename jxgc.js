      ...
      try {
        if (err) {
          ...
          if (safeGet(data)) {
            ...
            if (data['ret'] === 0) {
              data = data['data'];
              $.unActive = true;//标记是否开启了NONE活动或者选购了商品进行生产
              $.encryptPin = '';
              $.shelvesList = [];
              if (data.factoryList && data.productionList) {
                ...
                that.log(`\n【NONE账号${$.index}（${$.UserName}）的${$.name}好友互助码】${data.user.encryptPin}`);
                ...
                $.unActive = false;//标记是否开启了NONE活动或者选购了商品进行生产
                if (!data.factoryList) {
                  that.log(`【提示】NONE账号${$.index}[${$.nickName}]NONE工厂活动未开始\n请手动去NONEAPP->游戏与互动->查看更多->NONE工厂 开启活动\n`);
                  // $.msg($.name, '【提示】', `NONE账号${$.index}[${$.nickName}]NONE工厂活动未开始\n请手动去NONEAPP->游戏与互动->查看更多->NONE工厂 开启活动`);
                } else if (data.factoryList && !data.productionList) {
                  that.log(`【提示】NONE账号${$.index}[${$.nickName}]NONE工厂未选购商品\n请手动去NONEAPP->游戏与互动->查看更多->NONE工厂 选购\n`)
                  let nowTimes = new Date(new Date().getTime() + new Date().getTimezoneOffset()*60*1000 + 8*60*60*1000);
                  if (nowTimes.getHours()  === 12) {
                    //如按每小时运行一次，则此处将一天推送2次提醒
                    $.msg($.name, '提醒⏰', `NONE账号${$.index}[${$.nickName}]NONE工厂未选择商品\n请手动去NONEAPP->游戏与互动->查看更多->NONE工厂 选择商品`);
                    // if ($.isNode()) await notify.sendNotify(`${$.name} - NONE账号${$.index} - ${$.nickName}`, `NONE账号${$.index}[${$.nickName}]NONE工厂未选择商品\n请手动去NONEAPP->游戏与互动->查看更多->NONE工厂 选择商品`)
                    if ($.isNode()) allMessage += `NONE账号${$.index}[${$.nickName}]NONE工厂未选择商品\n请手动去NONEAPP->游戏与互动->查看更多->NONE工厂 选择商品${$.index !== cookiesArr.length ? '\n\n' : ''}`
                  }
                }
              }
            } else {
  ...
  if (exchangeEndTime) {
    ...
    if (nowTimes < exchangeEndTime) {
      ...
      if ((exchangeEndTime - nowTimes.getTime()) <= 3600000 * 2) {
        ...
        $.msg($.name, ``, `【NONE账号${$.index}】${$.nickName}\n【生产商品】${$.productName}${expiredTime}分钟后兑换超时\n【兑换截止时间】${$.exchangeEndTime}\n请速去NONEAPP->首页->好物0元造进行兑换`, {'open-url': jxOpenUrl, 'media-url': $.picture})
        // if ($.isNode()) await notify.sendNotify(`${$.name} - NONE账号${$.index} - ${$.nickName}`, `【NONE账号${$.index}】${$.nickName}\n【生产商品】${$.productName}${(exchangeEndTime - nowTimes) / 60*60*1000}分钟后兑换超时\n【兑换截止时间】${$.exchangeEndTime}\n请速去NONEAPP->首页->好物0元造进行兑换`, { url: jxOpenUrl })
        if ($.isNode()) allMessage += `【NONE账号${$.index}】${$.nickName}\n【生产商品】${$.productName}${expiredTime}分钟后兑换超时\n【兑换截止时间】${$.exchangeEndTime}\n请速去NONEAPP->首页->好物0元造进行兑换${$.index !== cookiesArr.length ? '\n\n' : ''}`
      }
...
function shareCodesFormat() {
  return new Promise(async resolve => {
    // that.log(`第${$.index}个NONE账号的助力码:::${$.shareCodesArr[$.index - 1]}`)
    $.newShareCodes = [];
    if ($.shareCodesArr[$.index - 1]) {
      $.newShareCodes = $.shareCodesArr[$.index - 1].split('@');
    } else {
      that.log(`由于您第${$.index}个NONE账号未提供shareCode,将采纳本脚本自带的助力码\n`)
    ...
    that.log(`第${$.index}个NONE账号将要助力的好友${JSON.stringify($.newShareCodes)}`)
    resolve();
  })
}
function requireConfig() {
  return new Promise(async resolve => {
    await updateTuanIdsCDN('https://wap.sogou.com/');
    ...
    const shareCodes = $.isNode() ? require('./jxgc.js') : '';
    that.log(`共${cookiesArr.length}个NONE账号\n`);
    $.shareCodesArr = [];
    if ($.isNode()) {
      ...
      that.log(`\nBoxJs设置的NONE工厂邀请码:${$.getdata('jd_jxFactory')}\n`);
    }
