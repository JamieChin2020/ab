...
//助力好友分享码(最多5个,否则后面的助力失败),原因:NONE农场每人每天只有四次助力机会
  ...
  try {
    ...
    if (initPetTownRes.code === '0' && initPetTownRes.resultCode === '0' && initPetTownRes.message === 'success') {
      ...
      if ($.petInfo.userStatus === 0) {
        // $.msg($.name, '', `【提示】NONE账号${$.index}${$.nickName}\n萌宠活动未开启\n请手动去NONEAPP开启活动\n入口：我的->游戏与互动->查看更多开启`, { "open-url": "openapp.jdmoble://" });
        await slaveHelp();//助力好友
        $.log($.name, '', `【提示】NONE账号${$.index}${$.nickName}\n萌宠活动未开启\n请手动去NONEAPP开启活动\n入口：我的->游戏与互动->查看更多开启`);
        return
      }
      if (!$.petInfo.goodsInfo) {
        $.msg($.name, '', `【提示】NONE账号${$.index}${$.nickName}\n暂未选购新的商品`, { "open-url": "openapp.jdmoble://" });
        if ($.isNode()) await notify.sendNotify(`${$.name} - ${$.index} - ${$.nickName}`, `【提示】NONE账号${$.index}${$.nickName}\n暂未选购新的商品`);
        return
      }
      ...
      if ($.petInfo.petStatus === 5) {
        ...
        $.msg($.name, `【提醒⏰】${$.petInfo.goodsInfo.goodsName}已可领取`, '请去NONEAPP或微信小程序查看', option);
        if ($.isNode()) {
          await notify.sendNotify(`${$.name} - 账号${$.index} - ${$.nickName || $.UserName}奖品已可领取`, `NONE账号${$.index} ${$.nickName}\n${$.petInfo.goodsInfo.goodsName}已可领取`);
        }
        return
      } else if ($.petInfo.petStatus === 6) {
        await slaveHelp();//已领取红包,但未领养新的,也能继续助力好友
        option['open-url'] = "openApp.jdMobile://";
        $.msg($.name, `【提醒⏰】已领取红包,但未继续领养新的物品`, '请去NONEAPP或微信小程序继续领养', option);
        if ($.isNode()) {
          await notify.sendNotify(`${$.name} - 账号${$.index} - ${$.nickName || $.UserName}奖品已可领取`, `NONE账号${$.index} ${$.nickName}\n已领取红包,但未继续领养新的物品`);
        }
        return
      }
      that.log(`\n【NONE账号${$.index}（${$.UserName}）的${$.name}好友互助码】${$.petInfo.shareCode}\n`);
      await taskInit();
    ...
    $.logErr(e)
    const errMsg = `NONE账号${$.index} ${$.nickName || $.UserName}\n任务执行异常，请检查执行日志 ‼️‼️`;
...
function shareCodesFormat() {
  return new Promise(async resolve => {
    // that.log(`第${$.index}个NONE账号的助力码:::${jdPetShareArr[$.index - 1]}`)
    newShareCodes = [];
