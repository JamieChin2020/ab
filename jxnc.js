...
!(async () => {
  ...
  for (let i = 0; i < $.cookieArr.length; i++) {
    ...
    if ($.currentCookie) {
      ...
      $.log(`\n开始【NONE账号${i + 1}】${userName}`);
      $.result.push(`【NONE账号${i + 1}】${userName}`);
...
function getCookies() {
  ...
  if (!$.cookieArr[0]) {
    $.msg($.name, '【提示】请先获取NONE账号一cookie\n直接使用NobyDa的NONE签到获取', 'https://bean.m.jd.com/', {
  ...
  return new Promise(async resolve => {
    $.get(taskUrl('query', `type=1`), async (err, resp, data) => {
      try {
        ...
        if (!$.info.active) {
          $.msg($.name, '请先去NONE农场选择种子！', '选择app专属种子时，请参考脚本头部说明获取token，点击通知跳转', { 'open-url': $.openUrl });
          resolve(false);
        }
        resolve(other);
      } catch (e) {
