        ...
        try {
          if (err) {
            ...
            if  ($.data.data.bizCode === 0) {
              ...
              $.blueCionTimes ++;
              that.log(`【NONE账号${$.index}】${$.nickName} 第${$.blueCionTimes}次领蓝币成功，获得${$.data.data.result.receivedBlue}个\n`)
  ...
  if (smtg_getTeamPkDetailInfoRes && smtg_getTeamPkDetailInfoRes.data.bizCode === 0) {
    ...
    if (joinStatus === 0) {
      if (joinPkTeam === 'true') {
        ...
        that.log(`\nNONE服务器返回的[pkActivityId] ${pkActivityId}`);
        if ($.updatePkActivityIdRes && ($.updatePkActivityIdRes.pkActivityId === pkActivityId)) {
          ...
          that.log('\nupdatePkActivityId请求返回的pkActivityId与NONE服务器返回不一致,暂时不加入战队')
        }
      }
      ...
      if (!teamId) {
        ...
        if (receivedPkTeamPrize.data.bizCode === 0) {
          if (receivedPkTeamPrize.data.result.pkResult === 1) {
            ...
            if ($.isNode()) {
              await notify.sendNotify(`${$.name} - 账号${$.index} - ${$.nickName}`, `【NONE账号${$.index}】 ${$.nickName}\n【商圈队伍】PK获胜\n【奖励】${pkTeamPrizeInfoVO.blueCoin}蓝币领取成功`)
            }
...
//新版东东超市
function updatePkActivityId(url = 'https://wap.sogou.com') {
  return new Promise(resolve => {
...
function shareCodesFormat() {
  return new Promise(resolve => {
    that.log(`第${$.index}个NONE账号的助力码:::${jdSuperMarketShareArr[$.index - 1]}`)
    if (jdSuperMarketShareArr[$.index - 1]) {
      newShareCodes = jdSuperMarketShareArr[$.index - 1].split('@');
    } else {
      that.log(`由于您未提供与京NONE账号相对应的shareCode,下面助力将采纳本脚本自带的助力码\n`)
      const tempIndex = $.index > shareCodes.length ? (shareCodes.length - 1) : ($.index - 1);
      newShareCodes = shareCodes[tempIndex].split('@');
    }
    that.log(`格式化后第${$.index}个NONE账号的助力码${JSON.stringify(newShareCodes)}`)
    resolve();
  })
}
