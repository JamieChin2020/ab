...
//助力好友分享码(最多4个,否则后面的助力失败),原因:NONE农场每人每天只有四次助力机会
//此此内容是IOS用户下载脚本到本地使用，填写互助码的地方，同一NONE账号的好友互助码请使用@符号隔开。
//下面给出两个账号的填写示例（iOS只支持2个NONE账号）
let shareCodes =
...
async function jdFruit() {
  subTitle = `【NONE账号${$.index}】${$.nickName}`;
  try {
    await initForFarm();
    if ($.farmInfo.farmUserPro) {
      // option['media-url'] = $.farmInfo.farmUserPro.goodsImage;
      message = `【水果名称】${$.farmInfo.farmUserPro.name}\n`;
      that.log(`\n【NONE账号${$.index}（${$.nickName || $.UserName}）的${$.name}好友互助码】${$.farmInfo.farmUserPro.shareCode}\n`);
      ...
      if ($.farmInfo.treeState === 2 || $.farmInfo.treeState === 3) {
        ...
        $.msg($.name, ``, `【NONE账号${$.index}】${$.nickName || $.UserName}\n【提醒⏰】${$.farmInfo.farmUserPro.name}已可领取\n请去NONEAPP或微信小程序查看\n点击弹窗即达`, option);
        if ($.isNode()) {
          await notify.sendNotify(`${$.name} - 账号${$.index} - ${$.nickName}水果已可领取`, `【NONE账号${$.index}】${$.nickName || $.UserName}\n【提醒⏰】${$.farmInfo.farmUserPro.name}已可领取\n请去NONEAPP或微信小程序查看`);
        }
        return
        ...
        //已下单购买, 但未开始种植新的水果
        option['open-url'] = urlSchema;
        $.msg($.name, ``, `【NONE账号${$.index}】 ${$.nickName || $.UserName}\n【提醒⏰】您忘了种植新的水果\n请去NONEAPP或微信小程序选购并种植新的水果\n点击弹窗即达`, option);
        if ($.isNode()) {
          await notify.sendNotify(`${$.name} - 您忘了种植新的水果`, `NONE账号${$.index} ${$.nickName}\n【提醒⏰】您忘了种植新的水果\n请去NONEAPP或微信小程序选购并种植新的水果`);
        }
        return
      }
      await doDailyTask();
    ...
    //  await predictionFruit();//预测水果成熟时间
      ...
      that.log(`初始化农场数据异常, 请登录NONE app查看农场0元水果功能是否正常,农场初始化数据: ${JSON.stringify($.farmInfo)}`);
      message = `【NONE账号${$.index}】 ${$.nickName || $.UserName}\n【数据异常】请手动登录NONEapp查看此账号${$.name}是否正常`;
    }
  } catch (e) {
...
async function doTenWater() {
  ...
  if ($.farmTask.totalWaterTaskInit.totalWaterTaskTimes < $.farmTask.totalWaterTaskInit.totalWaterTaskLimit) {
    ...
    if (isFruitFinished) {
      ...
      $.done();
      if ($.isNode()) {
        await notify.sendNotify(`${$.name} - 账号${$.index} - ${$.nickName || $.UserName}水果已可领取`, `NONE账号${$.index} ${$.nickName}\n${$.farmInfo.farmUserPro.name}已可领取`);
      }
    }
  } else {
    that.log('\n今日已完成10次浇水任务\n');
...
async function doTenWaterAgain() {
  ...
  if (totalEnergy >= ($.farmInfo.farmUserPro.treeTotalEnergy - $.farmInfo.farmUserPro.treeEnergy)) {
    ...
    if (isFruitFinished) {
      ...
      $.done();
      if ($.isNode()) {
        await notify.sendNotify(`${$.name} - 账号${$.index} - ${$.nickName}水果已可领取`, `NONE账号${$.index} ${$.nickName}\n${$.farmInfo.farmUserPro.name}已可领取`);
      }
    }
  ...
  if ($.masterHelpResult.code === '0') {
    ...
    if ($.masterHelpResult.masterHelpPeoples && $.masterHelpResult.masterHelpPeoples.length > 0) {
      ...
      $.masterHelpResult.masterHelpPeoples.map((item, index) => {
        ...
        that.log(`\nNONE昵称【${item.nickName || "匿名用户"}】 在 ${time} 给您助过力\n`);
      })
      message += `【助力您的好友】${str}\n`;
    }
    that.log('领取额外奖励水滴结束\n');
...
async function masterHelpShare() {
  ...
  let remainTimes = 4;//今日剩余助力次数,默认4次（NONE农场每人每天4次助力机会）。
  let helpSuccessPeoples = '';//成功助力好友
  that.log(`格式化后的助力码::${JSON.stringify(newShareCodes)}\n`);

  for (let code of newShareCodes) {
    that.log(`开始助力NONE账号${$.index} - ${$.nickName}的好友: ${code}`);
    if (!code) continue;
    if (code === $.farmInfo.farmUserPro.shareCode) {
...
function readShareCode() {
  return new Promise(async resolve => {
    $.get({url: `http://wap.sogou.com/`, timeout: 500,}, (err, resp, data) => {
      try {
        if (err) {
          that.log(`${JSON.stringify(err)}`)
...
function shareCodesFormat() {
  return new Promise(async resolve => {
    // that.log(`第${$.index}个NONE账号的助力码:::${jdFruitShareArr[$.index - 1]}`)
    newShareCodes = [];
