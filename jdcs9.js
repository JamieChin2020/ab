...
let coinToBeans = '20'; //兑换多少数量的京豆（20或者1000），0表示不兑换，默认兑换20京豆，如需兑换把0改成20或者1000，或者'商品名称'(商品名称放到单引号内)即可
      ...
      $.post(url, async (err, resp, data) => {
        try {
          ...
          if (safeGet(data)) {
            ...
            if ($.data.data.bizCode !== 0) {
              $.beanerr = `${$.data.data.bizMsg}`;
              //that.log(`【NONE账号${$.index}】${$.nickName} 换取京豆失败：${$.data.data.bizMsg}`)
              return
            }
            if ($.data.data.bizCode === 0) {
              if (`${coinToBeans}` === '1000') {
                $.beanscount ++;
                that.log(`【NONE账号${$.index}】${$.nickName} 第${$.data.data.result.exchangeNum}次换${$.title}成功`)
...
function msgShow() {
  ...
  return new Promise(async resolve => {
    ...
    if ($.beanscount && ctrTemp) {
      ...
      if ($.isNode()) {
        await notify.sendNotify(`${$.name} - 账号${$.index} - ${$.nickName}`, `【NONE账号${$.index}】${$.UserName}\n${coinToBeans ? `【兑换${$.title}】${$.beanscount ? `成功，数量：${$.beanscount}个` : $.beanerr}` : "您设置的是不兑换奖品"}`)
      }
    }
    resolve()
  })
